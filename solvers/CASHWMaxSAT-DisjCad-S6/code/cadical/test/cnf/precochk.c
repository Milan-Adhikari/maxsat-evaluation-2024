/*
 * Copyright (c) 2009, Armin Biere, JKU, Linz, Austria.  All rights
 * reserved.
 */
#include <assert.h>
#include <ctype.h>
...
SKIP1:
  ...
  if ((ch != 'S' && ch != 'U') ||
      (ch == 'S' &&
       (gzgetc (solution) != 'A' || gzgetc (solution) != 'T' ||
        gzgetc (solution) != 'I' || gzgetc (solution) != 'S' ||
        gzgetc (solution) != 'F' || gzgetc (solution) != 'I' ||
        gzgetc (solution) != 'A' || gzgetc (solution) != 'B' ||
        gzgetc (solution) != 'L' || gzgetc (solution) != 'E')) ||
      (ch == 'U' &&
       (gzgetc (solution) != 'N' || gzgetc (solution) != 'S' ||
        gzgetc (solution) != 'A' || gzgetc (solution) != 'T' ||
        gzgetc (solution) != 'I' || gzgetc (solution) != 'S' ||
        gzgetc (solution) != 'F' || gzgetc (solution) != 'I' ||
        gzgetc (solution) != 'A' || gzgetc (solution) != 'B' ||
...
SKIP2:
  ...
  if (ch != 'p')
    die ("expected 'c' or 'p'");
  if (gzgetc (dimacs) != ' ' || gzgetc (dimacs) != 'c' ||
      gzgetc (dimacs) != 'n' || gzgetc (dimacs) != 'f' ||
      gzgetc (dimacs) != ' ')
    die ("invalid header");
  ch = gzgetc (dimacs);
  if (!isdigit (ch))
