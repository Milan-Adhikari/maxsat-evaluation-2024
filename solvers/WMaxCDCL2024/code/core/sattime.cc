    ...
    for (j = 0; j < c.size(); j++)
    {
        ...
        for (int k = 0; k < ks.size(); k++)
        {
            ...
            if (kcls.lastSatVar() != redundant)
            {
                ...
                if ((int)kcls.nbTrue() == ca[kcls.cr].size())
                {
                    cls.setLastSatVar(redundant);
                    return newRedundant;
                }
                if (j == c.size() - 1 && (int)kcls.nbTrue() == c.size())
                {
...
void Solver::initialize() {
    ...
    for (i = 0; i < staticNbVars; i++)
    {
        ...
        if (value(i) == l_Undef)
        {
            ...
            if (score[i] > 0 && (!isSoftLits(i) || softScore(i) > 0))
                decrVars.push(i);
        }
        // if (isSoftLits(i) &&
...
void Solver::check_implied_clauses(Var v) {
    ...
    for (i = 0; i < vs.size(); i++)
    {
        ...
        if (level(neibor_v) > 0)
        {
            ...
            if (score[neibor_v] <= 0 && score[neibor_v] + tmp_score[neibor_v] > 0 && (!isSoftLits(neibor_v) || softScore(neibor_v) > 0))
                // if ((score[neibor_v] <= 0 && score[neibor_v] + tmp_score[neibor_v] > 0) ||
                // 	(isSoftLits(neibor_v) && score[neibor_v] < 0 && score[neibor_v] + tmp_score[neibor_v] >= 0))
...
int64_t Solver::softScore(int v) {
    return isSoftLits(v) ? (assignsLS[v] == sign(softLits[v]) ? weights[v] : -weights[v]) : 0;
}
...
void Solver::collectClauses(vec<CRef>& clauseSet, int learntType) {
    ...
    for (i = 0, j = 0; i < clauseSet.size(); i++)
    {
        ...
        if (removed(cr))
            continue;
        Clause& c = ca[cr];
        if (c.learnt() && (int)c.mark() != learntType)
            continue;
        if (cleanClause(cr))
        {
            if (c.learnt())
...
bool Solver::simpleStrengthenClause(CRef cr, Lit l) {
    ...
    else
    {
        ...
        {// && value(c[0]) == l_True && level(var(c[0])) == 0) {
...
bool Solver::backwardSubsume() {
    ...
    int mySavedTrail = trail.size(), savedOriginal;
...
bool Solver::sattime(int maxsteps) {
    ...
    if (!backwardSubsume() || !eliminateEqLits_(prevEquivLitsNb, true))
        return false;
    //if (!backwardSubsume() || !eliminateEqLits())
    //    return false;
            ...
            if (unSAT.size() == 0)
            {
                ...
                if (cost < UB)
                {
                  ...
                  //		assigns[v]=assignsLS[v]?l_True:l_False;
                    printf("c Solution improved by LS: %" PRId64 " in flip %d\n", UB, j);
