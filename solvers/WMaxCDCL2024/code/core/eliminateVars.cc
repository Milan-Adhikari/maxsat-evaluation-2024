...
bool Solver::crossResolvents(vec<CRef>& pos, vec<CRef>& neg, Var v)
{
    ...
    sort(small, clauseSize_lt(ca));
    sort(great, clauseSize_lt(ca));
    ...
    for (int i = 0; i < small.size(); i++)
    {
        ...
        printf("\nc small(%d/%d), with v=%d: ", i, small.size(), v);
        printClause(ca[cr]);
        printf("-------------------------------------------\n");
#endif
...
#ifdef printResolvent
                            printf("c resolvent from dr and cr: ");
                            printClause(resolvent2);
#endif
                            addClauseByResolution(resolvent2);
        ...
        else
        {
            ...
            cancelUntilRecord(trailRecord);
#ifdef printResolvent
            printf("c resolvent from cr: ");
            printClause(resolvent1);
#endif
            nb1stClauseSimp++;
            nbSavedResolv += great.size();
...
bool Solver::crossResolventsL(vec<CRef>& posL, vec<CRef>& negL, Var v)
{
    if (value(v) != l_Undef || posL.size() == 0 || negL.size() == 0)
        return true;
    //  add_tmp.clear();
    sort(posL, clauseLBDsize_lt(ca));
    sort(negL, clauseLBDsize_lt(ca));
                    ...
                    if (res)
                    {
                        ...
                        else
                        {
                                    ...
                                    (resLength <= clause_limit)))
                            {
                                ...
                                printClause(resolvent2);
#endif
                                addClauseByResolution(resolvent2, newlbd);
                                //bool added = addClauseByResolution(resolvent2, newlbd);
                                //assert(added);
                            }
#ifdef printResolventL
                            else
                                printf("c toooo long resolvent (%d, %d+%d) from cr and dr:\n",
...
{
    //printf("c enter eliminateVar with v %d\n", v);
...
bool Solver::eliminate_()
{
    ...
    while (!elim_heap.empty() || trail_size_last < trail.size())
    {
        ...
        while (!elim_heap.empty())
        {
            ...
            if (isEliminated(elim) || value(elim) != l_Undef || dynVar(elim) || isSoftLits(elim))
                continue;
...
void Solver::buildElimHeap()
{
    ...
    for (Var v = 0; v < staticNbVars; v++)
        if (!isSoftLits(v) && value(v) == l_Undef && (!isEliminated(v)) &&
            (n_occ[toInt(mkLit(v))] > 0 || n_occ[toInt(~mkLit(v))] > 0))
        {
            vs.push(v);
            curr_activity[v] = activity[v];
        }
    elim_heap.build(vs);
#ifdef printElimVarsData
...
bool Solver::eliminate(bool checkOcc)
{
    ...
    {
        occurIn[i].clear();
        occurInLocal[i].clear();
        n_occ[toInt(mkLit(i))] = 0;
        n_occ[toInt(~mkLit(i))] = 0;
    }
    ...
    printf("c nbElim %d, original clauses %d, learnts_core %d, learnts_tier2 %d, PBC %d, learnts_local %d, hardens %d, trail %d\n",
        nbElim, clauses.size(), learnts_core.size(), learnts_tier2.size(), PBC.size(), learnts_local.size(), hardens.size(), trail.size());
#endif
    ...
    for (grow = -2; grow < growLimit && s_ticks - saved_s_ticks <= simplifyTicksLimit; grow += 2)
    {
        ...
        printf("c %d(%d) 1stClauseSimp, %d(%d) nbSavedResolv, s_up " PRIu64 ", s_ticks " PRIu64 "\n",
    ...
    printf("c hardens %d, PBC %d, isetClauses %d\n\n",
        hardens.size(), PBC.size(), isetClauses.size());
    ...
    printf("c avgRemovedLits %4.3lf, nbUnits %d, s_up " PRIu64 ", elimTime %5.2lfs, s_ticks " PRIu64 "\n",
...
bool Solver::simplifyResolvents()
{
    ...
    for (ci = 0; ci < resolvents.size() && (starts > 0 || s_propagations - saved_s_up <= simplifyUPlimit); ci++)
    {
        ...
        if (cleanClause(cr))
        {
            ...
            else
            {
                ...
                {
                    shortened++;
                    nbRemovedLits += beforeSize - afterSize;
                }
    ...
    //   printf("c avgRemovedLits %4.3lf, nbUnits %d, s_up "PRIu64"\n",
...
bool Solver::simplebackwardSubsume_(int& savedTrailSize, bool verbose)
{
    ...
    if (verbose)
    {
        ...
        printf("c allsbsm " PRIu64 ", sbsmTtlg " PRIu64 " (%5.4f), sbsmAbsFail " PRIu64 "(%5.4f)\n", allsbsm, sbsmTtlg, rate1, sbsmAbsFail, rate2);
    }
    // #ifndef NDEBUG
    if (verbose)
    {
